...
def install() -> int:
    ...
    while (proceed := input("\nYou are about to \033[4minstall\033[m the DWARF INDI driver."
    ...
    for device_label in ["DWARF 3", "DWARF mini", "DWARF 2"]:
        ...
        if existing is None:
            device_elem = xml.SubElement(dev_group, "device", {"label": device_label, "manufacturer": "DWARFLAB"})
...
def uninstall():
    ...
    while (proceed := input("\nYou are about to \033[3;4mun\033[m\033[4minstall\033[m the DWARF"
                            " INDI driver. Proceed? [Y/n] ").lower()) not in ("y", "yes", "n", "no"):
        print(f"Unrecognized input '{proceed}'. Enter 'yes' or 'no'.")
    if proceed.startswith("n"):
        print("Aborting without action.\n")
        return 0
    ...
    try:
        driver_xml: xml._ElementTree = xml.parse(xml_definition_file.as_posix())
        dev_group: xml._Element = driver_xml.find(f"devGroup[@group='Telescopes']")
        for device_label in ["DWARF 3", "DWARF mini", "DWARF 2"]:
            existing = dev_group.find(f"device[@label='{device_label}']")
            if existing is None:
                print(f"- Driver for '{device_label}' is not currently present in {xml_definition_file}")
            else:
                dev_group.remove(existing)
